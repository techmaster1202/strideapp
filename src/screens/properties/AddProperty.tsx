...
const AddPropertyScreen = ({navigation}: UpdateUserScreenProps) => {
  ...
  const [is_str, setIsStr] = useState(false);
  ...
  const onSubmit = async (data: PropertyDetailFormData) => {
    ...
    data.accommodation_size = data.accommodation_size || 0;
    data.entrance_code = (data.entrance_code || 0).toString();
    data.square_feet = data.square_feet || 'need';
    ...
    data.price_paying_cleaning = (data.price_paying_cleaning || 0).toString();
    data.ical_url = data.ical_url || 'need';
    data.attachments = attachments;
    data.pets_allowed = data.pets_allowed || false;
    data.laundry_needed = data.laundry_needed || false;
    data.washer_dryer_on_site = data.washer_dryer_on_site || false;
    data.recurring = '0';
        ...
        <View style={[globalStyles.container]}>
          <View style={{width: '100%', marginBottom: 20}}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <Text>Is this a STR?</Text>
              <Checkbox
                status={is_str === true ? 'checked' : 'unchecked'}
                onPress={() => setIsStr(p => !p)}
              />
            </View>
            ...
            <Text style={{marginVertical: 5, fontWeight: '500'}}>Address</Text>
            <Controller
              control={control}
              rules={{}}
              render={({field: {onChange, onBlur, value}}) => (
