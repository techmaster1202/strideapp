...
import {UpdateUserScreenProps} from '../../types/index.ts';
...
import {getRole, updateRole} from '../../services/rolesService.ts';

const UpdateRoleScreen = ({route, navigation}: UpdateUserScreenProps) => {
  ...
  const onSubmit = async (data: {name: string; permissions: number[]}) => {
    ...
    setLoading(true);
    await updateRole(id, data.name, data.permissions)
      .then(response => {
        Toast.show({
  ...
  useEffect(() => {
    getRole(id).then(res => {
      ...
      // Pre-check permissions matching role.permissions
      const initialChecked = roleData.permissions.reduce(
        (acc: {[key: number]: boolean}, p: Record<string, any>) => {
          acc[p.id] = true;
          return acc;
        },
        {},
      ...
      setValue(
        'permissions',
        roleData.permissions.map((p: Record<string, any>) => p.id),
      );
    });
  }, []);
  ...
  const toggleCheckbox = (id2: number) => {
    setCheckedItems(prevState => {
      const checked = !prevState[id2];

      // Update form permissions based on toggle
      const currentPermissions = getValues('permissions');
      const updatedPermissions = checked
        ? [...currentPermissions, id2] // Add if checked
        : currentPermissions.filter(permId => permId !== id2); // Remove if unchecked
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      <DetailPageHeader
        ...
        title={AppConstants.TITLE_UpdateRole}
      />
      <ScrollView>
